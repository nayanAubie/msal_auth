...
/// Msal Auth Stream Handler for handling communication between Flutter and iOS.
@available(iOS 14.0, *)
class MsalAuthStreamHandler: NSObject, FlutterStreamHandler {
    /// Static property to store the Flutter event sink for communication.
    private static var eventSink: FlutterEventSink?

    /// Initializes the MsalStreamHandler with the provided Flutter plugin registrar.
    /// - Parameter registrar: The Flutter plugin registrar.
    public required init(registrar: FlutterPluginRegistrar) {
        super.init()
        let eventChannel = FlutterEventChannel(name: "msal_auth/event",
    ...
    {
        MsalAuthStreamHandler.eventSink = eventSink
        return nil
    }
    ...
    public static func sendEvent(type: String, data: [String: Any] = [:]) {
        guard let eventSink = MsalAuthStreamHandler.eventSink else {
            return
        }
    ...
    public func onCancel(withArguments _: Any?) -> FlutterError? {
        MsalAuthStreamHandler.eventSink = nil
        return nil
    }
}
